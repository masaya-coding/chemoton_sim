...
#------fat storing mechanism--------------------------------------------------------------------------
    ...
    hunger = (X <= no_food) and (not has_met_B or Z <= no_food)
...
#----------------------------------------------------------------
    ...
    def simulate_step(self, dt, method='rk4'):
        ...
        has_met_B = self.state[HAS_MET_B_IDX] > 0.5
        
        hungry = (X < no_food) and (not has_met_B or Z < no_food)
        ...
        if hungry:
            ...
            if current_food > self.prev_food:
                # good direction → keep going (with small randomness)
                dx = self.prev_dx + random.choice([-1, 0, 1])
                dy = self.prev_dy + random.choice([-1, 0, 1])
            else:
                # bad direction → change direction
                dx = random.choice([-1, 0, 1])
                dy = random.choice([-1, 0, 1])

            # clip to valid movement
            self.move_dx = max(-1, min(1, dx))
            self.move_dy = max(-1, min(1, dy))

            # store for next step
            self.prev_dx = self.move_dx
            self.prev_dy = self.move_dy
